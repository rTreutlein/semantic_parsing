...
class PLN2NL_Signature(dspy.Signature):
  ...
  pln: list[str] = dspy.InputField(desc="The PLN to be converted to NL")
  query: list[str] = dspy.InputField(desc="Query that produced the PLN, don't try to answer it just use it for context")
  similar_examples: list[str] = dspy.OutputField(desc="Examples of similar PLN statments and how they were converted to NL")
  previous_sentences: list[str] = dspy.OutputField(desc="Previous sentences used to generate the new statements")

class NL2PLN_Signature(dspy.Signature):
  """
